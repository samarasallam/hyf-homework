...
let theOdds = numbers.filter(oddNumbers); 
console.log("The odd numbers is: " + theOdds); 

function doubled(theOdds){
	return theOdds *2; 
}
let doubledNum = theOdds.map(doubled);
console.log("Doubled odd numbers: " + doubledNum); 
...
let eighties = movies.filter(function (movie){
	return movie.year >= 1980 && movie.year <=1989 ;
}); 

console.log("Numbers of Movies made between 1980 and 1989 is:"+ eighties.length);


//2 adding the key tag based on rating 
let moviesWithTags = movies.map(movie => {

		let ratingTag = movie.rating < 4 ? "Bad" : movie.rating >= 7 ? "Good" : "Average" ;

		
	return movie.tag = ratingTag;
})

console.log(moviesWithTags);
console.log( moviesWithTags[1].tag)


//3 filter the array to contain the movies rated higher than 6,map the movies array to only the rating of the movies.
let highRated = movies.filter(function (movie){
	return movie.rating > 6; 
})

let onlyRating= highRated.map(function (mov){
	return mov.rating; 
})
console.log("movies rated higher than 6 is " + onlyRating.length + " / They are : " + onlyRating); 
...
let averageRating = onlyRating.reduce( function (total,num){
	return total + num 
} , 0);  
console.log("The average rating for all movies :" + averageRating.toFixed(2)); 


// 6 Count the total number of Good, Average and Bad movies 
